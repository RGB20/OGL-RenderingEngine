 ...
 */
float perlinNoiseAccumulated(float x, float y, int octaves = 2);




/**
 * Interpolates value between range a and b. Returns a value between [0,1].
 *
 * @param a The range min.
 * @param b The range max.
 * @return a number between [0,1] for an input value and range [a,b].
 */
...
public :
	PerlinNoise() 
	{
		...
		distrib = std::uniform_real_distribution<>(0, 99);
		MakePermutation();
	}

	/**
	 * Generates Perlin noise at the given coordinates (x, y).
	 *
	 * @param x The x-coordinate for which Perlin noise is generated.
	 * @param y The y-coordinate for which Perlin noise is generated.
	 * @return The Perlin noise value at the specified coordinates.
...
private:
	...
	// Create a random number engine, seeded for different sequences
	std::random_device rd; // Provides a non-deterministic seed if available
	std::mt19937 gen;

	// Define a uniform integer distribution for numbers between 0 and 99
	std::uniform_real_distribution<> distrib;

	std::vector<int> arrayToShuffle;
